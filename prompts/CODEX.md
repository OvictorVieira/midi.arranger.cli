...
Este driver e entregue ao Codex por stdin via `codex exec -`, com a raiz do projeto ja fixada pelo
harness. Nao dependa de sessao anterior nem de confirmacoes interativas.
...
- `report.build`: use DEPOIS de `render` (e de `compliance.validate`) para gravar `arrangement-report.json` — o relatorio de proveniencia que liga fonte pesquisada -> achado -> mapeamento (`influence.compile`) -> tecnica autorizada -> track/secao carimbada -> metrica medida no MIDI final e veredito dos validadores. Passe `influence` e `brief_path` para a cadeia fechar; elo ausente sai declarado em `missing_links`, nunca suposto. `aplicada_verificada` so aparece quando um validador POR TRACK cobriu aquela track E a tecnica esta autorizada no brief desta execucao — NAO relate tecnica como aplicada com sucesso com base em outra coisa. Persona, colisao e conformidade sao de escopo global e nao cobrem track nenhuma; erro deles rebaixa todo mundo. Leia `techniques.aplicadas_verificadas` (ou `techniques.por_status`), nunca `techniques.aplicadas` sozinha: essa lista junta verificada, nao verificavel, com erro e sem autorizacao.
